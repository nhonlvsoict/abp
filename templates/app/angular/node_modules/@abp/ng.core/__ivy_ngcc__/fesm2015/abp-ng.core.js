...
class AuthPasswordFlowStrategy extends AuthFlowStrategy {
    ...
    loginCaptcha(params) {
        const tenant = this.sessionState.getTenant();
        return from(this.oAuthService.fetchTokenUsingPasswordRecaptchaFlow(params.username, params.password, params.twoFactorCode, new HttpHeaders(Object.assign({}, (tenant && tenant.id && { [this.tenantKey]: tenant.id }))))).pipe(this.pipeToLogin(params));
    }
    verifyreCaptcha(recaptcha) {
        const tenant = this.sessionState.getTenant();
        return from(this.oAuthService.verifyreCaptcha(recaptcha, new HttpHeaders(Object.assign({}, (tenant && tenant.id && { [this.tenantKey]: tenant.id })))));
    }
    pipeToLogin(params) {
        const router = this.injector.get(Router);
...
class AuthService {
    ...
    verifyreCaptcha(params) {
        return this.strategy.verifyreCaptcha(params);
    }
}
